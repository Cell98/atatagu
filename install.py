...
rDownloadURL = {"main": "http://192.168.3.9/Nova/main.tar.gz "sub": "http://192.168.3.9/Nova/sub.tar.gz"}
...
def prepare(rType="MAIN"):
    ...
    os.system("wget -q -O /tmp/libpng12.deb http://192.168.3.9/Nova/libpng12-0_1.2.54-1ubuntu1_amd64.deb")
...
def encrypt(rHost="127.0.0.1", rUsername="user_iptvipro", rPassword="", rDatabase="xtream_iptvipro", rServerID=1, rPort=7999):
    printc("Encrypting...")
...
def start(first=True):
    if first: printc("Starting Services")
    else: printc("Restarting Services")
...
def modifyNginx():
    ...
    if not "listen 10000;" in rPrevData:
...
def installAdmin():
    printc("Instalando Admin Painel")
    os.system("apt-get install unzip e2fsprogs python-paramiko -y > /dev/null")
    os.system("chattr -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null")
    os.system("rm -rf /home/xtreamcodes/iptv_xtream_codes/admin")
    os.system("rm -rf /home/xtreamcodes/iptv_xtream_codes/pytools")
    os.system("wget http://192.168.3.9/Nova/22F.zip -O /tmp/update.zip -o /dev/null")
    os.system("unzip -qq /tmp/update.zip -d /tmp/update/")
    os.system("cp -rf /tmp/update/XtreamUI-master/* /home/xtreamcodes/iptv_xtream_codes/")
...
if __name__ == "__main__":
    printc("IPTV SCRIPTs PAINEL - Instalar", col.OKGREEN, 2)
    ...
    if rType.upper() in ["MAIN", "LB"]:
        if rType.upper() == "LB":
            ...
            rPassword = generate()
            rServerID = 1
        rUsername = "user_iptvipro"
        rDatabase = "xtream_iptvipro"
        rPort = 7999
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            printc("Iniciar a instalação? Y/N", col.WARNING)
            if raw_input("  ").upper() == "Y":
                ...
                if rType.upper() == "MAIN":
                    installAdmin()
                start()
                printc("Instalação completa!", col.OKGREEN, 2)
                if rType.upper() == "MAIN":
                    printc("Admin UI: http://%s:10000" % getIP())
                    printc("Guarde sua senha do MySQL!")
                    printc(rPassword)
            else: printc("Instalação cancelada", col.FAIL)
